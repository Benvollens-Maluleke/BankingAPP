    ...
    if request.method == 'POST':
        ...
        for account_number, user in users.items():
            if user.get('username', '').lower() == username.lower():
                ...
                save_users(users)  # Save updated users to the file
...
def transaction_history():
    ...
    user = find_user(account_number)
    if user:
        if isinstance(user['transaction_history'], list):
            filtered_transactions = [t for t in user['transaction_history'] if isinstance(t, dict) and t.get('amount', 0) > 0]
...
def transfer():
    ...
    if request.method == 'POST':
        ...
        if recipient_account == str(account_number):  # Restrict transfer to own account
        ...
        if user:
            if user['balance'] >= amount:
                ...
                flash(f"Transferred R{amount} to account {recipient_account}. New balance: R{user['balance']}", 'success')
            else:
                flash("Insufficient funds", 'error')
